...
#if VECT_WIDTH==16
#define FLOAT_VECT uint16
unsigned int sub_sum(FLOAT_VECT * tmp_Ax, FLOAT_VECT * tmp_x)
{
	__private FLOAT_VECT tmp_sum=0;
//parallel mul
...
//reduction
	__private unsigned int tmp_red0,tmp_red1,tmp_red2,tmp_red3,tmp_red4,tmp_red5,tmp_red6,tmp_red7,tmp_red8,tmp_red9,tmp_red10,tmp_red11,tmp_red12,tmp_red13;
...
void spmv(__global unsigned int* Ax,
		__global int* Aj,
		__global int* Ap,
		...
		unsigned int num_rows)
{
	...
	{     
		...
		for (jj = row_start; jj < row_end; jj=jj+NUM_PAR)
		{
			...
			{
	//#pragma unroll UNROLL_F
				__attribute__((opencl_unroll_hint(UNROLL_F)))
